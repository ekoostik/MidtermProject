		...
		<table>
			...
			<tbody>
				<tr>
					...
					<td>
					${loggedinuser.household.occupants}" />
						</td>
						<td><c:choose>
								<c:when test="${empty loggedinuser.household.vehicles}">No Vehicles Listed<br>
									<a href="updatePet.do" class="btn btn-secondary" role="button">Add
										Vehicle</a>
								</c:when>
								<c:otherwise>
						...
						<td><c:choose>
								<c:when test="${!empty loggedinuser.household.pets}">
									<c:forEach var="p" items="${loggedinuser.household.pets}">
										<form action="petUpdated.do" method="POST">
										  <input type="hidden" name="id" value="${p.id}" /><br>
										  Name <input type="text" name="name" value="${p.name}"/><br>
										  Species <input type="text" name="species" value="${p.species}"/><br>
										  Breed <input type="text" name="breed" value="${p.breed}"/><br>
										  Color <input type="text" name="color" value="${p.color}"/><br>
										  <label>Tagged?</label> 
										  	<select name="tagged">
